<div class="space-y-8">
    ...
    @if($showForm)
        <div class="bg-white p-6 rounded-2xl border-2 border-blue-100 shadow-sm animate-fadeIn">
            ...
            <form wire:submit.prevent="save" class="space-y-6">
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" wire:model="code" class="w-full p-2 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:ring focus:ring-blue-200 outline-none transition uppercase" placeholder="Ex: PIONNIER20">
                        @error('code') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Type</label>
                        <select wire:model="type" class="w-full p-2 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:ring focus:ring-blue-200 outline-none transition">
                ...
                <div class="grid md:grid-cols-3 gap-6">
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Valeur</label>
                        <input type="number" step="0.01" wire:model="value" class="w-full p-2 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:ring focus:ring-blue-200 outline-none transition">
                        @error('value') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Date d'expiration</label>
                        <input type="datetime-local" wire:model="expires_at" class="w-full p-2 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:ring focus:ring-blue-200 outline-none transition">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Nombre max d'utilisations</label>
                        <input type="number" wire:model="max_uses" class="w-full p-2 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:ring focus:ring-blue-200 outline-none transition" placeholder="Illimité si vide">
                    </div>
                </div>
                ...
                <div class="flex items-center gap-6 border-t pt-6">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        ...
                        <span class="text-sm font-bold text-slate-700 group-hover:text-blue-600 transition">Code Actif</span>
                    </label>
                </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 border-b-2 border-slate-200">
                    <th class="p-4 text-sm font-bold text-slate-600 uppercase tracking-wider">Code</th>
                    <th class="p-4 text-sm font-bold text-slate-600 uppercase tracking-wider">Réduction</th>
                    <th class="p-4 text-sm font-bold text-slate-600 uppercase tracking-wider">Validité</th>
                    <th class="p-4 text-sm font-bold text-slate-600 uppercase tracking-wider">Utilisations</th>
                    <th class="p-4 text-sm font-bold text-slate-600 uppercase tracking-wider text-right">Actions</th>
                </tr>
            </thead>
                ...
                @forelse($coupons as $coupon)
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="p-4 text-sm">
                            <span class="font-bold text-slate-700">{{ $coupon->uses_count }}</span>
                            @if($coupon->max_uses)
