    ...
    @if($showForm)
        <div class="bg-white p-6 rounded-lg shadow-md border-t-4 border-blue-600 transition-all">
            ...
            <div class="md:col-span-2 flex justify-end gap-2">
                <button type="button" wire:click="resetFields" class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600 flex items-center transition-colors">
                    <x-fas-times class="w-4 h-4 mr-2" />
                    Annuler
                </button>
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 flex items-center transition-colors shadow-sm">
                ...
                @foreach($projectTypes as $type)
                    <tr>
                        <td class="px-6 py-4 font-medium flex items-center">
                            ...
                            {{ $type->name }}
                            @if($type->is_default)
                                <span class="ml-2 px-2 py-0.5 text-[10px] font-bold bg-green-100 text-green-700 rounded-full flex items-center">
                                    <x-fas-check-circle class="w-2.5 h-2.5 mr-1" />
                                    Défaut
                                </span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            @if(!$type->is_default)
                                <button wire:click="setAsDefault({{ $type->id }})" class="text-green-600 hover:text-green-900 inline-flex items-center text-sm font-semibold" title="Définir par défaut">
                                    <x-fas-star class="w-4 h-4 mr-1" />
                                    Par défaut
                                </button>
                            @endif
                            <button wire:click="edit({{ $type->id }})" class="text-blue-600 hover:text-blue-900 inline-flex items-center text-sm font-semibold">
